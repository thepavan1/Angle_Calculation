...
while True:
    ...
    if results.pose_landmarks:
        ...
        # Calculate hip center point (midpoint between left and right hips)
        hip_center = None
        if J(L_HIP) is not None and J(R_HIP) is not None:
            hip_center = (J(L_HIP) + J(R_HIP)) / 2.0
        ...
        for name, (ia, ib, ic) in angle_defs.items():
            ...
            # Filter and display geometric angle only
            f_angle = get_filter(name).filter(t, raw_ang)
            cv2.putText(frame, f"{name}: {f_angle:.1f} deg", (10, sidebar_y),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 0), 2)
            sidebar_y += 22
        ...
        # NEW: Calculate and display angle between legs (left ankle - hip center - right ankle)
        if hip_center is not None and J(L_ANK) is not None and J(R_ANK) is not None:
            left_ankle = J(L_ANK)
            right_ankle = J(R_ANK)
            
            # Calculate vectors from hip center to ankles
            v1 = left_ankle - hip_center
            v2 = right_ankle - hip_center
            
            # Calculate the angle between the legs at the hip center
            denom = (np.linalg.norm(v1) * np.linalg.norm(v2) + 1e-9)
            cosang = np.clip(np.dot(v1, v2) / denom, -1.0, 1.0)
            legs_angle = math.degrees(math.acos(cosang))
            
            # Smooth the angle
            f_legs_angle = get_filter("Legs_Angle").filter(t, legs_angle)
            
            # Display legs angle
            cv2.putText(frame, f"Legs_Angle: {f_legs_angle:.1f} deg", (10, sidebar_y),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 0, 255), 2)
            sidebar_y += 22
            
            # Draw visualization for legs angle
            try:
                # Convert 3D points to pixel coordinates
                hip_center_px = (int((hip_center[0] / (calib.mpp if calib.calibrated else 1.0) + cx)),
                                int((cy - hip_center[1] / (calib.mpp if calib.calibrated else 1.0))))
                left_ankle_px = (int((left_ankle[0] / (calib.mpp if calib.calibrated else 1.0) + cx)),
                                int((cy - left_ankle[1] / (calib.mpp if calib.calibrated else 1.0))))
                right_ankle_px = (int((right_ankle[0] / (calib.mpp if calib.calibrated else 1.0) + cx)),
                                 int((cy - right_ankle[1] / (calib.mpp if calib.calibrated else 1.0))))
                
                # Draw lines from hip center to ankles
                cv2.line(frame, hip_center_px, left_ankle_px, (255, 0, 255), 2)
                cv2.line(frame, hip_center_px, right_ankle_px, (255, 0, 255), 2)
                
                # Draw angle arc at hip center
                draw_angle_arc(frame, left_ankle_px, hip_center_px, right_ankle_px, (255, 0, 255))
                
                # Display angle value near hip center
                cv2.putText(frame, f"{legs_angle:.0f} deg", 
                           (hip_center_px[0] + 8, hip_center_px[1] - 8),
                           cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 1)
                
            except Exception as e:
                print(f"Error drawing legs angle visualization: {e}")
